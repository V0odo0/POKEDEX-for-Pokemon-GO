...
public class PokeMovePage : PokePanel
{
    ...
    public void Set(PrimaryMove move)
    {
        ...
        _moveStats.SetStats(move.AttackRate, move.CooldownRate, move.DpsRate);
        _moveStatsText.text =
            move.Attack +
            UIManager.NewLine(1) +
            move.Cooldown + " " + UIManager.GetFormattedText(TextManager.GetText(TextType.Element, 16), TextColorType.GreyLight, false, 12) +
            UIManager.NewLine(1) +
            move.Dps.ToString("F") + UIManager.GetFormattedText(" / " + TextManager.GetText(TextType.Element, 16), TextColorType.GreyLight, false, 12);

        _typeChart.Set(TypeChartPanelType.Move, move.StrongAgaints, move.WeakAgains);

        _otherStatsCountText.text = 
            move.EnergyGain.ToString() + UIManager.GetFormattedText(" / 100 ", TextColorType.GreyLight, false, 12);
    }
