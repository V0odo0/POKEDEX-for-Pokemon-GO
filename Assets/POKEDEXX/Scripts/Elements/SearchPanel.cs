...
public class SearchPanel : PokePanel
{
    ...
    void OnEnable()
    {
        ...
        _holderText.text = 
            ...
            AppManager.Instance.PokeData.PokeInfos[Random.Range(0, AppManager.Instance.PokeData.PokeInfos.Length)].Name;
    }
    ...
    public void OnEndEdit()
    {
        ...
        PokeInfo[] foundedPokeInfos = TrySearchPokeInfos(_inputText.text.ToLower());
    ...
    PokeInfo[] TrySearchPokeInfos(string inputText)
    {
        List<PokeInfo> foundedPokeInfos = new List<PokeInfo>();

        #region Find by Name
        PokeInfo[] allPokeInfos = AppManager.Instance.PokeData.PokeInfos;
        for (int i = 0; i < allPokeInfos.Length; i++)
        {
            if (allPokeInfos[i].Name.ToLower().Contains(inputText))
        ...
        #region Fin by Type
        List<PokeInfo> withTypeList = new List<PokeInfo>();

        List<PokeType> selectedTypes = new List<PokeType>();
        for (int i = 0; i < _typeSelections.Length; i++)
        {
            if (_typeSelections[i].Selected)
                selectedTypes.Add((PokeType) _typeSelections[i].ElementId);
        }
        ...
        #region Find by Egg
        List<PokeInfo> withEggList = new List<PokeInfo>();
