...
public class PokemonList : PokePanel
{
    ...
    private PokeInfo[] _curPokeInfos;
    ...
    public void SetList(PokeInfo[] pokeInfos)
    {
    ...
    {
        List<PokeInfo> pokeInfos = _curPokeInfos.ToList();
        ...
        switch (CurSortMethod)
        {
            ...
            case SortMethod.ByName:
                if (CurSortOrder == SortOrder.Inc)
                    SetList(pokeInfos.OrderBy(info => info.Name).ToArray());
                else SetList(pokeInfos.OrderByDescending(info => info.Name).ToArray());
                break;
    ...
    {
        PokeInfo[] pokeInfos = _pokeInfoPages[pageId].PokeInfos.ToArray();
    ...
    public void OnPokemonClick(int id)
    {
        ...
        UIManager.Instance.ShowPokemonPage(Mathf.Clamp(--id, 0, AppManager.Instance.PokeData.PokeInfos.Length - 1));
    }
